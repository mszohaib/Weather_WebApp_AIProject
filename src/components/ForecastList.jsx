    ...
    <div className="forecast-list">
      ...
      {fiveDayForecast && fiveDayForecast.length > 0 ? (
        fiveDayForecast.map((item, index) => {
          ...
          //Returning the jsx element that are suppossed to deal with it for eavery item for each element
          return (
            <div className="forecast-items" item={item} key={index}>
              {/* The html t  ags that you want to display */}
              <h3> Day -{index + 1}</h3>
              {/* To Convert the date and time into humand weekday only*/}
              <h3>{dayName}</h3>
              {/* To display the date*/}
              <h3>
